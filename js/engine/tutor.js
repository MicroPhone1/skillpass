...
export function ask(question, { trackId = null, skillId = null } = {}){
  ...
  return {
    ...
    blocks, sources,
    ...coachFor(question, top.chunk),
    hits: hits.map(h => ({ id:h.chunk.id, title:h.chunk.title, score:+h.score.toFixed(2) })),
  };
}

/* ============================================================
   ส่วนโค้ช — ทำให้คำตอบ "พาคิด" ไม่ใช่ "จ่ายคำตอบ"

   ฝั่ง AI สั่งด้วย prompt ได้ แต่เอนจินในเครื่องต้องสร้างเอง
   และตัวนี้คือตัวที่เดโมสาธารณะใช้จริง (ไม่มีคีย์ AI)
   จึงประกอบจากข้อมูลที่มีอยู่แล้วเท่านั้น ไม่ใช่ประโยคสำเร็จรูปลอย ๆ
   ============================================================ */

/** จับว่าเป็นคำถามแนวคำนวณไหม เพื่อไม่ยกตัวเลขให้ฟรี ๆ */
const isCalcQuestion = s =>
  /คำนวณ|คิดยังไง|กี่|เท่าไหร่|เท่าไร|ขนาดเท่า|สูตร/.test(String(s));

export function coachFor(question, chunk){
  const topic = chunk?.title || 'เรื่องนี้';

  const probe = isCalcQuestion(question)
    ? `ก่อนดูคำตอบ ลองไล่ก่อนว่าโจทย์ให้ค่าอะไรมาบ้าง และสูตรของ “${topic}” ต้องการค่าอะไรที่ยังขาดอยู่`
    : `ก่อนดูคำตอบ ลองตอบตัวเองก่อนว่า “${topic}” เกี่ยวข้องกับสิ่งที่คุณถามตรงจุดไหน`;

  const nextTime = isCalcQuestion(question)
    ? 'ครั้งหน้าเริ่มจากเขียนสิ่งที่โจทย์ให้มาก่อน แล้วค่อยเลือกสูตร ไม่ใช่จำคำตอบของโจทย์ข้อนี้'
    : `ครั้งหน้าลองค้นหัวข้อ “${topic}” ในคู่มือก่อนถาม จะจำโครงเรื่องได้เร็วกว่าอ่านคำตอบสำเร็จรูป`;

  const verify = chunk?.source?.ref
    ? `ก่อนเอาไปใช้หน้างาน เปิด ${chunk.source.ref} ตรวจตัวเลขกับเงื่อนไขด้วยตัวเองอีกครั้ง`
    : 'ก่อนเอาไปใช้หน้างาน ตรวจกับคู่มือฉบับจริงอีกครั้ง ระบบนี้เป็นตัวช่วยซ้อม ไม่ใช่ตัวตัดสิน';

  return { probe, nextTime, verify };
}
...
export function explainQuestion(q, { given = null, correct = null } = {}){
  ...
  const skill = skillName(q.track, q.skill);
  const firstStep = (q.steps || [])[0];

  return {
    kind:'answer',
    lead: correct === false
      ? `ข้อนี้เกี่ยวกับ “${skill}” — มาดูวิธีคิดที่ถูกต้องกันครับ`
      : `เก่งมาก! ทวนวิธีคิดของข้อ “${skill}” อีกรอบให้แม่นขึ้น`,
    blocks,
    /* ตอบผิดให้ย้อนไปหาขั้นที่พลาด ตอบถูกให้ดันต่ออีกขั้น
       เพราะคนที่ตอบถูกด้วยการเดา กับคนที่เข้าใจจริง ต้องแยกออกจากกันให้ได้ */
    probe: correct === false
      ? `ก่อนเปิดดูวิธีทำ ลองย้อนดูเองก่อนว่าคำตอบที่เลือกไปพลาดตั้งแต่ขั้นไหน${
          firstStep ? ` — เริ่มจากถามตัวเองว่า “${firstStep}” ทำครบหรือยัง` : ''}`
      : `ตอบถูกแล้ว ลองดันต่ออีกขั้น — ถ้าเปลี่ยนเงื่อนไขในโจทย์ไปหนึ่งอย่าง คำตอบจะเปลี่ยนไปทางไหน และเพราะอะไร`,
    nextTime: firstStep
      ? `ครั้งหน้าเจอโจทย์ “${skill}” ให้เริ่มจาก ${firstStep} เสมอ แล้วค่อยไล่ขั้นถัดไป — จำวิธี ไม่ใช่จำคำตอบ`
      : `ครั้งหน้าเจอโจทย์ “${skill}” ให้ไล่ทีละขั้นตามวิธีข้างบน อย่าข้ามไปเดาคำตอบ`,
    verify: q.sources?.[0]?.ref
      ? `เทียบกับ ${q.sources[0].ref} ด้วยตัวเองอีกครั้ง อย่าเชื่อคำอธิบายนี้อย่างเดียว`
      : 'เทียบกับคู่มือเตรียมสอบฉบับจริงอีกครั้งก่อนนำไปใช้',
