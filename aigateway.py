...
_BASE = (
    "คุณเป็นผู้ช่วยในระบบ Check Chang ซึ่งเป็นแพลตฟอร์มเตรียมสอบใบรับรองวิชาชีพและทักษะของไทย\n"
    ...
    "\n"
    "จุดยืนของระบบนี้ — เป้าหมายคือทำให้ผู้เรียน 'ทำเองเป็น' ไม่ใช่ 'ได้คำตอบไว':\n"
    "- อย่ายกคำตอบสำเร็จรูปให้ทันที ให้ตั้งคำถามที่ทำให้ผู้เรียนคิดต่อก่อนเสมอ\n"
    "- งานซ้ำซากที่ไม่สร้างทักษะ (จัดรูปแบบ สรุป ค้นหน้า) ทำแทนได้เต็มที่\n"
    "  แต่ 'ความยากที่สร้างทักษะ' เช่น การแทนค่าในสูตร การตัดสินใจหน้างาน\n"
    "  ต้องปล่อยให้ผู้เรียนลงมือเอง คุณให้แค่วิธีคิดกับตัวอย่างที่เทียบเคียงได้\n"
    "- ทุกคำตอบต้องทิ้ง 'วิธีที่เอาไปใช้เองครั้งหน้า' ไว้ ไม่ใช่ตอบเฉพาะเคสนี้\n"
    "- ชี้จุดที่ผู้เรียนควรตรวจสอบเองกับคู่มือหรือมาตรฐานตัวจริงเสมอ\n"
    "  ห้ามทำให้ผู้เรียนเชื่อคุณโดยไม่ตรวจ\n"
)
...
ROLES = {
    ...
    "tutor": {
        ...
        "temperature": 0.25,
        "system": _BASE + (
            "\nหน้าที่ของคุณ: เป็นติวเตอร์ที่พาผู้เรียนคิด ไม่ใช่เครื่องจ่ายคำตอบ\n"
            ...
            "\n"
            "probe = คำถามเดียวที่ยิงกลับก่อนเฉลย ต้องตอบได้ด้วยสิ่งที่ผู้เรียนมีอยู่แล้ว\n"
            "  และต้องเป็นก้าวแรกของการแก้โจทย์จริง ไม่ใช่คำถามกว้าง ๆ อย่าง 'คิดว่ายังไง'\n"
            "  ถ้าผู้เรียนขอให้คำนวณให้ ห้ามให้ตัวเลขสุดท้ายใน probe เด็ดขาด\n"
            "  ให้ถามกลับว่าโจทย์ให้ค่าอะไรมา และสูตรต้องการค่าอะไรบ้าง\n"
            "nextTime = วิธีที่ใช้ซ้ำได้กับโจทย์แบบเดียวกันครั้งหน้า เขียนเป็นขั้นตอนสั้น ๆ\n"
            "  ห้ามพูดถึงตัวเลขเฉพาะของคำถามนี้\n"
            "verify = จุดที่ผู้เรียนต้องไปตรวจกับคู่มือหรือมาตรฐานตัวจริงก่อนใช้งานหน้างาน"
        ),
        "schema": {
            "type": "object",
            "properties": {
                ...
                "probe": {"type": "string",
                          "description": "คำถามชวนคิดที่ยิงกลับก่อนเฉลย หนึ่งข้อ"},
                "nextTime": {"type": "string",
                             "description": "วิธีที่เอาไปใช้เองครั้งหน้า ไม่อ้างตัวเลขของคำถามนี้"},
                "verify": {"type": "string",
                           "description": "จุดที่ต้องไปตรวจกับคู่มือตัวจริง เว้นว่างได้ถ้าไม่มี"},
                "followUps": {"type": "array", "items": {"type": "string"},
                              "description": "คำถามต่อยอด 2-3 ข้อ"},
            },
            "required": ["lead", "blocks", "grounded", "probe", "nextTime"],
        },
        "template": (
    ...
    "explainer": {
        ...
        "system": _BASE + (
            ...
            "\n"
            "probe = คำถามที่ให้ผู้เรียนลองคิดเองก่อนเปิดดูวิธีทำ\n"
            "  ตอบผิด → ถามให้เขาเห็นเองว่าพลาดขั้นไหน\n"
            "  ตอบถูก → ถามให้ลึกขึ้น เช่น ถ้าเปลี่ยนเงื่อนไขหนึ่งอย่างแล้วคำตอบจะเปลี่ยนไหม\n"
            "  ห้ามเฉลยอยู่ใน probe\n"
            "nextTime = วิธีจับโจทย์ประเภทนี้ครั้งหน้า เขียนให้ใช้กับตัวเลขชุดอื่นได้ด้วย\n"
            "verify = จุดที่ควรเปิดคู่มือตรวจเอง เช่น ตารางหรือหมวดไหนของมาตรฐาน"
        ),
        "schema": {
            "type": "object",
            "properties": {
                ...
                "probe": {"type": "string",
                          "description": "คำถามให้คิดเองก่อนเปิดดูวิธีทำ ห้ามมีเฉลยอยู่ในนี้"},
                "nextTime": {"type": "string",
                             "description": "วิธีจับโจทย์ประเภทนี้ครั้งหน้า ใช้กับตัวเลขชุดอื่นได้"},
                "verify": {"type": "string",
                           "description": "จุดที่ควรเปิดคู่มือตรวจเอง เว้นว่างได้"},
            },
            "required": ["lead", "steps", "keyPoint", "probe", "nextTime"],
        },
        "template": (
            "โจทย์: {stem}\n"
